# --- VERSION: m_commander_v8_2_Auto_Build_Edition ---
import gspread
...
async def execute_single_analysis(doc, conf):
    ...
    for i, u in enumerate(valid_units):
        c_idx = i + 8
        diffs_total = unit_history[u]; days_total = len(diffs_total)
        period_avgs = []
        for p_idx in range(3):
            ...
            else: period_avgs.append("-")
        
        avg_total = int(sum(diffs_total)/days_total) if days_total > 0 else 0
        summary_header[0][c_idx] = period_avgs[0]
        summary_header[1][c_idx] = period_avgs[1]
        summary_header[2][c_idx] = period_avgs[2]
        summary_header[3][c_idx] = f"{avg_total}枚"
        summary_header[4][c_idx] = f"{len([v for v in diffs_total if v>=5000])/days_total*100:.1f}%" if days_total > 0 else "0.0%"
        summary_header[5][c_idx] = f"{len([v for v in diffs_total if v>=10000])/days_total*100:.1f}%" if days_total > 0 else "0.0%"
    ...
    for i, d_str in enumerate(target_dates):
        ...
        row = [d_str, dow_names[datetime.strptime(d_str, "%Y/%m/%d").weekday()], "", t_d, int(t_d/u_cnt), int(t_g/u_cnt), f"{(t_g*3+t_d)/(t_g*3)*100 if t_g>0 else 100.0:.1f}%", f"{sticky_val:.1f}%"]
    ...
    print(f"\n   -> {conf['target_model']} 解析完了 (Version 8.2 Build)")

async def main():
    print(f"\n--- Ver.8.2 起動 (Auto Build Edition) ---")
