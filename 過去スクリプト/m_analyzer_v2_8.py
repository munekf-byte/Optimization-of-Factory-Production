# --- VERSION: m_analyzer_v2.8_Three_Waves_Edition_20260129 ---
...
async def run_analysis():
    ...
    # ==========================================
    # BLOCK: 5. 視覚化魔法（3段階 波状攻撃）
    # ==========================================
    s_id = cross_ws.id
    l_row = len(final_rows)
    l_col = len(header)
    
    # 第1波：基本デザイン（絶対に失敗しない部分）
    print("--- 3. 第1波：基本デザインを適用中... ---")
    base_reqs = [
    ...
    # 第2波：色彩ルール（勝敗の可視化）
    print("--- 4. 第2波：色彩ルールを適用中... ---")
    color_reqs = []
    midpoint = 100 if conf['mode'] == "機械割" else 0
    for c_idx in range(2, l_col):
        color_reqs.append({"addConditionalFormatRule": {"rule": {"ranges": [{"sheetId": s_id, "startRowIndex": 2, "endRowIndex": l_row, "startColumnIndex": c_idx, "endColumnIndex": c_idx+1}],
            "booleanRule": {"condition": {"type": "NUMBER_GREATER", "values": [{"userEnteredValue": str(midpoint)}]},
            "format": {"backgroundColor": hex_to_rgb("#cfe2f3"), "textFormat": {"foregroundColor": hex_to_rgb("#0000ff")}}}}, "index": 0}})
        color_reqs.append({"addConditionalFormatRule": {"rule": {"ranges": [{"sheetId": s_id, "startRowIndex": 2, "endRowIndex": l_row, "startColumnIndex": c_idx, "endColumnIndex": c_idx+1}],
            "booleanRule": {"condition": {"type": "NUMBER_LESS", "values": [{"userEnteredValue": str(midpoint)}]},
            "format": {"backgroundColor": hex_to_rgb("#f4cccc"), "textFormat": {"foregroundColor": hex_to_rgb("#ff0000")}}}}, "index": 0}})
    doc.batch_update({"requests": color_reqs})

    # 第3波：グラフ生成（最もエラーが起きやすい部分）
    print("--- 5. 第3波：グラフを生成中... ---")
    try:
        min_v, max_v = (75, 120) if conf['mode'] == "機械割" else (-2000, 2500)
        series = []
        for c_idx in range(6, 14): # MA(7)とMA(15)の計8本
            series.append({"series": {"sourceRange": {"sources": [{"sheetId": s_id, "startRowIndex": 1, "endRowIndex": l_row, "startColumnIndex": c_idx, "endColumnIndex": c_idx+1}]}}, "targetAxis": "LEFT_AXIS"})
                    ...
                    "spec": {
                        ...
                        "basicChart": {
                            ...
                            "axis": [
                                ...
                                {"position": "LEFT_AXIS", "title": conf['mode'], "viewWindowOptions": {"viewWindow": {"min": min_v, "max": max_v}, "viewWindowMode": "EXPLICIT"}}
                            ],
        ...
        doc.batch_update({"requests": graph_req})
        print("【成功】グラフの描画に成功しました。")
    except Exception as ge:
        print(f"【警告】グラフの描画のみ失敗しました（理由はAPIの制限等）: {ge}")
        print("表のデータと色彩はすでに反映されています。")

    print(f"\n--- 分析完了！スプレッドシートを確認してください ---")
