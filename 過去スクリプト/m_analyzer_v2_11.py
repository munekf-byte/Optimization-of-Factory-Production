# --- VERSION: m_analyzer_v2.11_Strategist_Palette_20260129 ---
...
async def run_analysis():
    ...
    print(f"--- 2. トレンド分析(MA7/15)を実行中... ({conf['mode']}) ---")
    daily_stats = {}
    for row in all_data[1:]:
        try:
    ...
    # 表データ構築 (3日MAはデータからも除外してスッキリさせます)
    ...
    # 色彩ルール
    for c_idx in range(2, 6): # 店全体・部門A・B・Cにのみ適用
    ...
    try:
        ...
        def build_chart_requests(title, start_col, anchor_row):
            # 店全体(Gray/Thick), A(Red/Thin), B(Blue/Thin), C(Green/Thin)
            colors = ["#cccccc", "#ff0000", "#0000ff", "#00ff00"]
            widths = [4, 2, 2, 2] # 店全体だけ太く
            
            series = []
            for i, c in enumerate(range(start_col, start_col + 4)):
                series.append({
                    "series": {"sourceRange": {"sources": [{"sheetId": s_id, "startRowIndex": 1, "endRowIndex": l_row, "startColumnIndex": c, "endColumnIndex": c+1}]}},
                    "targetAxis": "LEFT_AXIS",
                    "color": hex_to_rgb(colors[i]),
                    "lineStyle": {"width": widths[i]}
                })
        ...
        graph_reqs = [
            build_chart_requests(f"【{conf['mode']}】7日移動平均トレンド", 6, 0),
            build_chart_requests(f"【{conf['mode']}】15日移動平均トレンド", 10, 18)
        ]
        doc.batch_update({"requests": graph_reqs})
        print("【成功】色彩と太さを最適化したグラフを生成しました！")
