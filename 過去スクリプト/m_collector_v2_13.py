# --- VERSION: m_collector_v2.13_20260128_Triple_Standard ---
...
# BLOCK: 1. 司令塔（設定エリア）
# ==========================================
# 収集したい期間を指定してください (YYYY-MM-DD)
START_DATE = "2025-09-01" # この日から
END_DATE   = "2026-01-27" # この日まで
...
TARGET_STORES = [
    ...
    {
        "active": True, # 使わない時は False にしてください
        "name": "マルハンつくば", 
    ...
    async with async_playwright() as p:
        ...
        for store in TARGET_STORES:
            ...
            try:
                ...
                print(f"期間内の未取得レポートを {len(tasks)} 件発見。")
                ...
                for task in tasks:
                    ...
                    if day_data:
                        ...
                        cal_sheet.append_row([task['date'], store['name'], len(rows), t_diff, int(t_diff/len(rows)), t_games])
                        
                        print(f"    -> {task['date']} 完了 / 差枚 {t_diff} / {len(rows)}台")
                    
                    # --- ステルス休憩：日次 ---
                    wait_time = random.uniform(2, 5)
                    print(f"    (休憩中: {wait_time:.1f}秒)")
                    await asyncio.sleep(wait_time)

                # --- ステルス休憩：店舗間 ---
                if store != [s for s in TARGET_STORES if s["active"]][-1]:
                    store_break = random.uniform(30, 60)
                    print(f"\n店舗間インターミッション中... {store_break:.1f}秒間待機します。")
