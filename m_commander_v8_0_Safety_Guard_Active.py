...
async def execute_single_analysis(doc, conf):
    ...
    if not valid_units: 
        print("   ! 有効な正規台（3/5ルール）が見つかりませんでした。")
        return
    ...
    for i, d_str in enumerate(target_dates):
        ...
        calc_rows.append([ma7, ma30, s_ma30]) # 検証用隔離データ

    ws.update(values=[data_header] + data_rows, range_name='A80')
    
    # 【改修】隔離データの書き込み位置（データ末尾から20列右）
    ...
    print(f"\n   -> {conf['target_model']} 解析完了 (Version 8.0 FIX)")

async def main():
    print(f"\n--- Ver.8.0 起動 (Safety Guard FIX) ---")
