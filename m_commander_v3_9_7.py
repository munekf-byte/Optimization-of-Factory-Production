# --- VERSION: m_commander_v3_9_7_Variable_Fix_20260205 ---
import gspread
...
async def sync_store_list(doc):
    print(f"   > [{datetime.now().strftime('%H:%M:%S')}] 倉庫スキャン中...")
    try:
        unique_stores = set()
        ...
        req = {"setDataValidation": {"range": {"sheetId": conf_ws.id, "startRowIndex": 4, "endRowIndex": 5, "startColumnIndex": 1, "endColumnIndex": 2}, "rule": {"condition": {"type": "ONE_OF_RANGE", "values": [{"userEnteredValue": range_formula}]}, "showCustomUi": True, "strict": True}}}
...
async def execute_cross_analysis(doc, conf):
    ...
    full_ws = doc.fetch_sheet_metadata(); current_sheet = next(s for s in full_ws['sheets'] if s['properties']['title'] == CROSS_SHEET)
    ...
    ws.update(values=[header_main] + data_rows, range_name='A80'); ws.update(values=data_rows, range_name='U80')
    ...
    reqs = [{"updateDimensionProperties": {"range": {"sheetId": s_id, "dimension": "COLUMNS", "startIndex": 6, "endIndex": 30}, "properties": {"pixelSize": 250}, "fields": "pixelSize"}}, {"repeatCell": {"range": {"sheetId": s_id, "startRowIndex": 79, "endRowIndex": l_row, "startColumnIndex": 2, "endColumnIndex": 18}, "cell": {"userEnteredFormat": {"numberFormat": get_number_format(conf['mode']), "horizontalAlignment": "CENTER"}}, "fields": "userEnteredFormat"}}, build_panorama_chart(0, 7, 0), build_panorama_chart(1, 15, 25), build_panorama_chart(2, 30, 50)]
    doc.batch_update({"requests": reqs})
    print("   -> クロス分析 完了")

# ==========================================
# BLOCK: 4. 機種別分析（エラー修正版）
# ==========================================
async def execute_single_analysis(doc, conf):
    ...
    summary_10k, summary_5k, summary_avg = ["10,000枚突破率", ""] + [""] * 5, ["5,000枚突破率", ""] + [""] * 5, ["台別平均差枚", ""] + [""] * 5
    ...
    data_rows = []
    for d_str in sorted_dates:
        day_data = model_data[d_str] # 【修正】ここでday_dataとして定義
        ...
        sticky = len([u for u in day_data.values() if u['games']>=5000 and u['diff']>0])/u_count
    ...
    ws.update(values=[summary_10k, summary_5k, summary_avg, header], range_name='A21'); ws.update(values=data_rows, range_name='A25')
    s_id = ws.id; l_row = len(data_rows) + 25; l_col = len(header)
    reqs = [{"repeatCell": {"range": {"sheetId": s_id, "startRowIndex": 20, "endRowIndex": 23}, "cell": {"userEnteredFormat": {"backgroundColor": hex_to_rgb("#f3f3f3"), "textFormat": {"bold": True}}}, "fields": "userEnteredFormat"}}]
    ...
    while True:
        try:
            ...
            if "実行" in str([all_cmd, single_cmd, cross_cmd]):
                ...
                conf = {"store": vals[4][1], "mode": vals[9][1], "target_model": vals[7][1], "A_name": vals[10][1], "A_list": [v[1] for v in vals[11:23] if v[1]], "B_name": vals[23][1], "B_list": [v[1] for v in vals[24:36] if v[1]], "C_name": vals[36][1], "C_list": [v[1] for v in vals[37:49] if v[1]]}
