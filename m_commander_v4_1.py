...
# ==========================================
# BLOCK: 3. 機種別分析（v4.1 プロフェッショナルUI）
# ==========================================
async def execute_single_analysis(doc, conf):
    ...
    for u, dates in unit_appearance.items():
        ...
        if len(dates) >= 3:
            for i in range(len(dates) - 2):
                if (dates[i+2] - dates[i]).days <= 4: is_valid = True; break
    ...
    data_rows = []
    col_vals = {'diff_total': [], 'diff_avg': [], 'games_avg': [], 'win_rate': []} # 相対評価用

    for d_str in sorted_dates:
        day_data = model_data[d_str]
        t_d, t_g, u_count = sum(u['diff'] for u in day_data.values()), sum(u['games'] for u in day_data.values()), len(day_data)
    ...
    for r_idx, vals in [(20, all_summary_vals['10k']), (21, all_summary_vals['5k'])]:
        ...
        if vals: # 上位30%を白抜きに
            ...
            reqs.append({"addConditionalFormatRule": {"rule": {"ranges": [{"sheetId": s_id, "startRowIndex": r_idx, "endRowIndex": r_idx+1, "startColumnIndex": 7, "endColumnIndex": l_col}], "booleanRule": {"condition": {"type": "NUMBER_GREATER_EQUAL", "values": [{"userEnteredValue": str(threshold)}]}, "format": {"textFormat": {"foregroundColor": {"red": 1, "green": 1, "blue": 1}, "bold": True}}}}, "index": 0}})

    # UI装飾3: 機械割 (絶対評価)
    f_range = {"sheetId": s_id, "startRowIndex": 24, "endRowIndex": l_row, "startColumnIndex": 5, "endColumnIndex": 6}
    rules = [
        ("NUMBER_LESS", ["90"], "#ff0000", True),
        ("NUMBER_BETWEEN", ["90", "100"], "#ff0000", False),
        ("NUMBER_BETWEEN", ["100", "103"], "#000000", False),
        ("NUMBER_BETWEEN", ["103", "110"], "#0000ff", False),
        ("NUMBER_GREATER_EQUAL", ["110"], "#0000ff", True)
    ]
    ...
    # UI装飾4: サマリー列 (C, D, E, G列) 相対5段階評価
    target_cols = [(2, 'diff_total'), (3, 'diff_avg'), (4, 'games_avg'), (6, 'win_rate')]
    for c_idx, key in target_cols:
        v_list = col_vals[key]
        ...
        fmt_rules = [
            ("NUMBER_LESS_EQUAL", [str(steps[0])], "#ff0000", True),
            ("NUMBER_BETWEEN", [str(steps[0]), str(steps[1])], "#ff0000", False),
            ("NUMBER_BETWEEN", [str(steps[1]), str(steps[2])], "#000000", False),
            ("NUMBER_BETWEEN", [str(steps[2]), str(steps[3])], "#0000ff", False),
            ("NUMBER_GREATER_EQUAL", [str(steps[3])], "#0000ff", True)
        ]
        for cond, vals, color, bold in fmt_rules:
    ...
    print("\n   -> 機種別分析 v4.1 完了 (相対・絶対評価UI適用)")

async def main():
    print(f"\n--- Ver.4.1 起動 (Intelligent UI Edition) ---")
