# --- VERSION: m_commander_v3_9_2_Strategy_Edition_20260205 ---
...
async def execute_single_analysis(doc, conf, all_data):
    ...
    for row in all_data[1:]:
        try:
            ...
            unit_int = int(d_unit)
            unique_units.add(unit_int)
            diff_int, games_int = int(d_diff), int(d_games)
            model_data[d_date][unit_int] = {'diff': diff_int, 'games': games_int}
            
            # 曜日別集計（レポート用）
            dow = datetime.strptime(d_date, "%Y/%m/%d").weekday()
            weekday_stats[dow].append(diff_int)
        except: continue
    ...
    report = [
        [f"【軍師レポート】 {conf['target_model']} 戦略指南"],
        [f"■ データから見えるこの店の傾向:"],
        [f"  - 最強曜日: {best_dow}曜日 (平均 {int(avg_by_dow[best_dow])}枚) → 優先的に狙う価値あり"],
        [f"  - 最弱曜日: {worst_dow}曜日 (平均 {int(avg_by_dow[worst_dow])}枚) → 回避または慎重な立ち回り推奨"],
        [f"■ 台別サマリー (21-23行目) を参照し、突破率の高い台をロックオンしてください。"]
    ]

    # --- サマリー行の計算 (21-23行目) ---
    summary_10k = ["10,000枚突破率", ""] + [""] * 5 # 前半7列
    summary_5k = ["5,000枚突破率", ""] + [""] * 5
    summary_avg = ["台別平均差枚", ""] + [""] * 5
    
    for u in sorted_units:
        u_diffs = [model_data[d][u]['diff'] for d in sorted_dates if u in model_data[d]]
        total_days = len(u_diffs)
        if total_days > 0:
            summary_10k.append(f"{round(len([v for v in u_diffs if v >= 10000])/total_days*100, 1)}%")
            summary_5k.append(f"{round(len([v for v in u_diffs if v >= 5000])/total_days*100, 1)}%")
            summary_avg.append(int(sum(u_diffs)/total_days))
        else:
            summary_10k.append("-"); summary_5k.append("-"); summary_avg.append("-")

    # --- 日次データの作成 (24行目〜) ---
    header = ["日付", "曜日", "機種総計", "台平均", "平均G", "機械割", "粘り勝率"] + [f"{u}番台" for u in sorted_units]
    data_rows = []
    for d_str in sorted_dates:
        ...
        data_rows.append([d_str, dow_names[datetime.strptime(d_str, "%Y/%m/%d").weekday()], 
                          t_d, int(avg_d), int(avg_g), m_rate, sticky] + 
                         [day[u]['diff'] if u in day else "" for u in sorted_units])

    # シート書き込み
    ws = doc.worksheet(SINGLE_SHEET); ws.clear()
    ws.update(values=report, range_name='A1')
    ws.update(values=[summary_10k, summary_5k, summary_avg, header] + data_rows, range_name='A21')
    
    s_id = ws.id; l_row = len(data_rows) + 25
    reqs = [
        {"repeatCell": {"range": {"sheetId": s_id, "startRowIndex": 0, "endRowIndex": 15}, "cell": {"userEnteredFormat": {"textFormat": {"bold": True, "fontSize": 11}, "backgroundColor": hex_to_rgb("#fff2cc")}}, "fields": "userEnteredFormat"}},
    ...
    reqs += [
        add_cf("=H22<=-3001","#f4cccc","#ff0000",True), add_cf("=AND(H22>=-3000,H22<=-1500)","#fff2cc","#ff0000",True),
        add_cf("=AND(H22>=2001,H22<=4999)","#cfe2f3","#000000",False), add_cf("=AND(H22>=5000,H22<=7999)","#6fa8dc","#ffffff",True), add_cf("=H22>=8000","#0b5394","#ffffff",True)
    ]
    doc.batch_update({"requests": reqs})
    print(f"   -> {conf['target_model']} 分析完了。")

# ==========================================
# BLOCK: 5. メインループ
# ==========================================
async def main():
    print(f"\n--- Ver.3.9.2 起動 (Strategy Edition) ---")
