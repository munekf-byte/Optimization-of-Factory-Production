# --- VERSION: m_commander_v3_6_1_Fixed_20260201 ---
import gspread
...
async def execute_cross_analysis(doc, conf, all_data):
    ...
    if conf['mode'] == "機械割": min_v, max_v, midpoint = 95.0, 110.0, 100.0
    elif conf['mode'] == "G数": min_v, max_v, midpoint = 1500.0, 6500.0, 4000.0
    else: min_v, max_v, midpoint = -600.0, 600.0, 0.0
    ...
    if charts:
        delete_reqs = [{"deleteEmbeddedObject": {"objectId": c['chartId']}} for c in charts]
        try: doc.batch_update({"requests": delete_reqs})
        except: pass
...
# (execute_single_analysis と main は v3.6 と同じため省略しますが、実運用時はそのまま含めてください)
...
async def main():
    print(f"\n--- Ver.3.6.1 起動 (BugFix) [{datetime.now().strftime('%H:%M:%S')}] ---")
