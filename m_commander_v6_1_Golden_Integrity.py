# --- VERSION: m_commander_v6_1_Golden_Integrity ---
# 1. v5.9安定エンジン回帰 (日付自動解析)
# 2. Y軸 90-110 グラフ固定
# 3. 粘り勝率 (H列) 算出ロジック復旧
# 4. 書式保護: A83以降のみ値消去 (81, 82の書式を死守)
...
async def execute_single_analysis(doc, conf):
    ...
    # 1. データ抽出 (v5.9方式: 店舗・機種に合致する全期間を自動対象とする)
    ...
    s_id = ws.id
    # 83行目以降のみ値を消去 (81-82の書式を守る)
    ws.batch_clear(['A83:ZZ2000'])
