# --- VERSION: m_commander_v3_8_2_Final_Fix_20260202 ---
    ...
    try:
        ...
        req = {
            "setDataValidation": {
                "range": {
                    "sheetId": conf_ws.id,
                    "startRowIndex": 4, "endRowIndex": 5, # B5セル
        ...
        print(f"   -> 同期完了: {len(unique_stores)}店舗反映。")
...
async def execute_cross_analysis(doc, conf, all_data):
    ...
    def build_panorama_chart(w_idx, window_val, anchor_row):
        ...
        chart_spec = {
            ...
            "basicChart": {
                ...
                "domains": [{"domain": {"sourceRange": {"sources": [{"sheetId": s_id, "startRowIndex": 49, "endRowIndex": l_row, "startColumnIndex": 20, "endColumnIndex": 21}]}}}],
                "series": series
            }
        }
        ...
        chart_position = {
            "overlayPosition": {
                "anchorCell": {"sheetId": s_id, "rowIndex": anchor_row, "columnIndex": 0},
                "widthPixels": 5000,
                "heightPixels": 450
            }
        }
        
        return {"addChart": {"chart": {"spec": chart_spec, "position": chart_position}}}
    ...
    reqs = [
        ...
        build_panorama_chart(0, 7, 0),
        build_panorama_chart(1, 15, 25),
        build_panorama_chart(2, 30, 50)
    ]
...
# ==========================================
async def main():
    print(f"\n--- Ver.3.8.2 起動 (Final Fix) [{datetime.now().strftime('%H:%M:%S')}] ---")
    ...
    while True:
        try:
            ...
            if "実行" in str([all_cmd, single_cmd, cross_cmd]):
                ...
                conf_ws.update_acell(btn_cell, "待機中"); print("   > 全工程完了。")
