# --- VERSION: m_commander_v5_94_Surgical_Edition ---
import gspread
...
# ==========================================
# BLOCK: 2. ユーティリティ (統計計算)
# ==========================================
def get_thresholds(values):
    """上位15%, 30%, 下位20% の境界値を算出"""
    nums = sorted([float(v) for v in values if v != "" and v is not None])
...
async def execute_single_analysis(doc, conf):
    ...
    try:
        ...
        print(f"   ! エラー: {TEMPLATE_SHEET} がありません。")
        return
    s_id = ws.id

    # 4. Dashboard A1:O33 (数値のみ埋め込み)
    ...
    # 6. 外科手術的書式整形 (Surgical Update)
    l_row, l_col = len(data_rows) + 81, len(data_header)
    reqs = []
    
    # [1] ベース書式の拡張 (81行目ヘッダ, 82行目データ色を全域へ)
    ...
    # [2] 相対評価に基づく文字色・太字の「上書き」
    # 各列の母集団から境界値を算出
    col_thresholds = {}
    for c in range(3, l_col): # D列から個別台末尾まで
    ...
    for i, row in enumerate(data_rows):
        ...
        reqs.append({
            "updateCells": {
                "range": {"sheetId": s_id, "startRowIndex": row_idx, "endRowIndex": row_idx + 1, "startColumnIndex": 3, "endColumnIndex": l_col},
                "rows": [{"values": row_formats}],
                "fields": "userEnteredFormat.textFormat" # 文字色と太字のみ。背景色は維持される。
            }
        })

    # [3] トレンドグラフ追加 (位置・サイズ・スケールは聖域準拠)
    ...
    print(f"\n   -> Version 5.94 完成 (Surgical Update 適用済)")
