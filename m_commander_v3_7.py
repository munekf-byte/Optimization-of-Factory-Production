# --- VERSION: m_commander_v3_7_Panorama_Scale_20260201 ---
...
# ==========================================
# BLOCK: 3. クロス分析エンジン（パノラマ・可変スケール版）
# ==========================================
async def execute_cross_analysis(doc, conf, all_data):
    print("   > クロス分析エンジン（可変スケール）始動...")
    ...
    def get_axis_limits(mode, window):
        if mode == "機械割":
            ...
            return 2500.0, 6500.0 # 15d, 30d
        else: # 差枚
            if window == 7:  return -2000.0, 1000.0
            return -600.0, 800.0 # 15d, 30d
        return -600.0, 600.0
    ...
    data_rows = []
    # グラフ用データ（U列以降）にリミッターをMA期間ごとに適用するため、複数のリミッターデータを作成
    chart_data_rows = [] 
    for i, d_str in enumerate(sorted_dates):
        ...
        chart_data_rows.append(row) # グラフ用は後で軸ごとにリミッターかける
    ...
    def build_panorama_chart(w_idx, window_val, anchor_row):
        ...
        return { "addChart": { "chart": { "spec": { "title": title, "basicChart": { "chartType": "LINE", "legendPosition": "BOTTOM_LEGEND",
                ...
                "widthPixels": 5000, "heightPixels": 380 } } } } } # 【重要】幅5000px
    ...
    reqs = [
        ...
        build_panorama_chart(1, 15, 20), # 重なり防止で20行飛ばし
        build_panorama_chart(2, 30, 40)  # 同上
    ]
    ...
    print(f"   -> パノラマ分析完了 (軸最適化済)")

# (execute_single_analysis と main は v3.6.1 を継承)
...
async def main():
    print(f"\n--- Ver.3.7 起動 (Panorama Edition) [{datetime.now().strftime('%H:%M:%S')}] ---")
    scope = ['https://spreadsheets.google.com/feeds', 'https://www.googleapis.com/auth/drive']
    creds = ServiceAccountCredentials.from_json_keyfile_name('credentials.json', scope)
    gc = gspread.authorize(creds)
    doc = gc.open_by_key(SPREADSHEET_KEY); conf_ws = doc.worksheet(CONFIG_SHEET)
    while True:
        try:
