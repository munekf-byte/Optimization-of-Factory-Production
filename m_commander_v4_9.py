...
async def execute_single_analysis(doc, conf):
    ...
    # 3. 個別台データ：5段階評価 & 空白グレー (BLANKに修正)
    for i, u in enumerate(valid_units):
        c_idx = i + 7
        u_vals = [v for v in unit_history[u] if v is not None]
        v_min, v_max = min(u_vals), max(u_vals); v_range = (v_max - v_min) or 1
        ...
        # 判定ルールの定義 (BLANKに修正)
        fmt_set = [
            ("BLANK", [], None, "#efefef", False), # 非稼働
            ("NUMBER_EQ", [str(v_max)], "#0000ff", None, True), # 最上
            ("NUMBER_GREATER_THAN_EQ", [str(steps[3])], "#0000ff", None, False), # 上
            ("NUMBER_LESS_THAN_EQ", [str(steps[0])], "#ff0000", None, True), # 最下
            ("NUMBER_LESS_THAN_EQ", [str(steps[1])], "#ff0000", None, False), # 下
        ]
    ...
    print("\n   -> 機種別分析 v4.9 完了 (All Fixed)")

async def main():
    print(f"\n--- Ver.4.9 起動 (API Keywords Fixed) ---")
