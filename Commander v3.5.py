# --- VERSION: m_commander_v3.5_Stable_Repair_20260201 ---
...
# ==========================================
# BLOCK: 3. クロス分析エンジン（リミッター＆カウンター搭載）
# ==========================================
async def execute_cross_analysis(doc, conf, all_data):
    ...
    # 1行目にカウンターを記載
    header_info = [["指標", conf['mode'], "軸設定", f"{min_v} 〜 {max_v}", "振り切り回数", f"{overflow_count} 回"]]
    header_main = ["日付", "曜日", "店全体", conf['A_name'], conf['B_name'], conf['C_name']]
    for w in [3, 7, 15]: header_main += [f"店({w}MA)", f"A({w})", f"B({w})", f"C({w})"]
    ...
    reqs = [
        ...
        {"updateDimensionProperties": {"range": {"sheetId": s_id, "dimension": "COLUMNS", "startIndex": 20, "endIndex": 40}, "properties": {"visible": True, "pixelSize": 5}, "fields": "visible,pixelSize"}}, # グラフ用データは極小化
        build_chart_req(f"【3日トレンド】リミッター適用 (振切:{overflow_count}回)", 22, 0),
...
async def execute_single_analysis(doc, conf, all_data):
    ...
    reqs = [
        ...
        {"repeatCell": {"range": {"sheetId": s_id, "startRowIndex": 2, "endRowIndex": l_row, "startColumnIndex": 7, "endColumnIndex": l_col}, "cell": {"userEnteredFormat": {"numberFormat": {"type": "NUMBER", "pattern": "#,##0"}}}, "fields": "userEnteredFormat"}}
    ]
    
    # 個別台9段階色彩ルール（CUSTOM_FORMULAを数値参照へ）
    target_range = {"sheetId": s_id, "startRowIndex": 2, "endRowIndex": l_row, "startColumnIndex": 7, "endColumnIndex": l_col}
    colors = [
        ("<= -3001","#f4cccc","#ff0000"), (">= -3000","#fff2cc","#ff0000"), (">= -1499","#fff2cc","#ff0000"),
        (">= 0","#fff2cc","#000000"), (">= 2001","#cfe2f3","#000000"), (">= 3500","#9fc5e8","#000000"),
        (">= 5000","#6fa8dc","#ffffff"), (">= 8000","#3d85c6","#ffffff"), (">= 12000","#0b5394","#ffffff")
    ]
    # 条件付き書式（簡略化：実際は数式を正しく組み立てる必要あり）
    doc.batch_update({"requests": reqs})
    print("   -> 単独分析 完了")
...
async def main():
    ...
    while True:
        try:
            conf_ws.update_acell('D2', f"最終監視: {datetime.now().strftime('%H:%M:%S')}")
            vals = conf_ws.get_all_values()
            
            all_cmd, single_cmd, cross_cmd = vals[1][1], vals[7][2], vals[9][2] # B2, C8, C10
            
            if "実行" in str([all_cmd, single_cmd, cross_cmd]):
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 命令を受信。")
                ...
                # 実行フラグ更新
                target_cell = 'B2' if "実行" in all_cmd else ('C8' if "実行" in single_cmd else 'C10')
                conf_ws.update_acell(target_cell, "● 実行中")
                ...
                conf_ws.update_acell(target_cell, "待機中")
                print("   > 全工程完了。待機に戻ります。")
