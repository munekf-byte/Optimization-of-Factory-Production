# --- VERSION: m_commander_v5_8_Zero_Division_Guard ---
import gspread
...
def split_periods(model_data, sorted_dates):
    ...
    return [p for p in periods if p][:3] # 空の期間を除去して最大3つ
...
async def execute_single_analysis(doc, conf):
    ...
    for d_str in target_dates:
        ...
        if day_units:
            avg_d_val = t_d / len(day_units)
            dow_stats[dt.weekday()].append(avg_d_val)
            digit_stats[dt.day % 10].append(avg_d_val)
    ...
    for i, p in enumerate(p_results):
        ...
        p_d_list = [model_data[d] for d in p['dates'] if d in model_data]
        if not p_d_list: continue
        p_ds_val = sum(sum(u['diff'] for u in day.values()) for day in p_d_list)
        p_gs_val = sum(sum(u['games'] for u in day.values()) for day in p_d_list)
        p_uc_val = sum(len(day) for day in p_d_list)
        
        dash[8+i][6], dash[8+i][7] = ["前期","中期","後期"][i], f"{p['dates'][0]}〜{p['dates'][-1]}"
        dash[8+i][9] = int(p_ds_val/p_uc_val) if p_uc_val > 0 else 0
        dash[8+i][10] = f"{(p_gs_val*3+p_ds_val)/(p_gs_val*3)*100:.1f}%" if p_gs_val > 0 else "100.0%"
        dash[8+i][11] = int(p_gs_val/p_uc_val) if p_uc_val > 0 else 0
    ...
    for i, d_str in enumerate(target_dates):
        ...
        if u_cnt == 0: continue # 稼働なし日はスキップ
        t_d_day, t_g_day = sum(u['diff'] for u in day_data.values()), sum(u['games'] for u in day_data.values())
        ma7_val = sum(payout_history[max(0, i-6):i+1])/len(payout_history[max(0, i-6):i+1])
        ma30_val = sum(payout_history[max(0, i-29):i+1])/len(payout_history[max(0, i-29):i+1])
        s_ma30_val = sum(store_payout_history[max(0, i-29):i+1])/len(store_payout_history[max(0, i-29):i+1])
        
        m_rate_val = (t_g_day*3+t_d_day)/(t_g_day*3)*100 if t_g_day > 0 else 100
        win_rate_val = (len([u for u in day_data.values() if u['games']>=5000 and u['diff']>0])/u_cnt*100)
        
        row = [d_str, dow_names[datetime.strptime(d_str, "%Y/%m/%d").weekday()], "", t_d_day, int(t_d_day/u_cnt), int(t_g_day/u_cnt), f"{m_rate_val:.1f}%", f"{win_rate_val:.1f}%"]
        for u in valid_units: row.append(day_data[u]['diff'] if u in day_data else "")
        row += ["", ma7_val, ma30_val, s_ma30_val]
    ...
    print("\n   -> Version 5.8 完了 (安定性向上)")

async def main():
    print(f"\n--- Ver.5.8 起動 (Stable Edition) ---")
