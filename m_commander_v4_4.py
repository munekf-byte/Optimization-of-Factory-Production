...
import csv
import collections
import jpholiday # 要: pip install jpholiday
...
# ==========================================
# BLOCK: 3. 機種別分析（v4.4 ハイブリッド評価 & 祝日対応）
# ==========================================
async def execute_single_analysis(doc, conf):
    ...
    for c_idx, key in [(2, 'diff_total'), (3, 'diff_avg'), (4, 'games_avg'), (6, 'win_rate')]:
        ...
        # ルール: 最大(青太), プラス(青細), 0(黒), マイナス(赤細), 最小(赤太)
        ...
        reqs.append({"addConditionalFormatRule": {"rule": {"ranges": [c_range], "booleanRule": {"condition": {"type": "NUMBER_GREATER", "values": [{"userEnteredValue": "0"}]}, "format": {"textFormat": {"foregroundColor": hex_to_rgb("#0000ff"), "bold": False}}}}, "index": 1}})
        ...
        reqs.append({"addConditionalFormatRule": {"rule": {"ranges": [c_range], "booleanRule": {"condition": {"type": "NUMBER_LESS", "values": [{"userEnteredValue": "0"}]}, "format": {"textFormat": {"foregroundColor": hex_to_rgb("#ff0000"), "bold": False}}}}, "index": 3}})

    # UI5: 【改善】真・日本暦カレンダー
    for i, d_str in enumerate(sorted_dates):
        dt = datetime.strptime(d_str, "%Y/%m/%d")
        color = {"red": 0, "green": 0, "blue": 0} # 黒
        if dt.weekday() == 5: color = {"red": 0, "green": 0, "blue": 1} # 土曜 青
        if dt.weekday() == 6 or jpholiday.is_holiday(dt): color = {"red": 1, "green": 0, "blue": 0} # 日曜・祝日 赤
    ...
    print("\n   -> 機種別分析 v4.4 完了 (日本暦・ハイブリッド色彩適用)")

async def main():
    print(f"\n--- Ver.4.4 起動 (Calendar & Hybrid UI) ---")
    ...
    while True:
        try:
            ...
            if "実行" in str([vals[1][1], vals[7][2]]):
                conf_ws.update_acell('B2' if "実行" in vals[1][1] else 'C8', "● 実行中")
                await execute_single_analysis(doc, {"store": vals[4][1], "target_model": vals[7][1]})
                conf_ws.update_acell('B2' if "実行" in vals[1][1] else 'C8', "待機中")
