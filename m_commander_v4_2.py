...
async def execute_single_analysis(doc, conf):
    ...
    data_rows = []
    col_vals = {'diff_total': [], 'diff_avg': [], 'games_avg': [], 'win_rate': []}
    ...
    for r_idx, vals in [(20, all_summary_vals['10k']), (21, all_summary_vals['5k'])]:
        ...
        if vals:
            threshold = sorted(vals)[-max(1, int(len(vals)*0.3))] # 上位30%
            reqs.append({"addConditionalFormatRule": {"rule": {"ranges": [{"sheetId": s_id, "startRowIndex": r_idx, "endRowIndex": r_idx+1, "startColumnIndex": 7, "endColumnIndex": l_col}], "booleanRule": {"condition": {"type": "NUMBER_GREATER_THAN_OR_EQUAL", "values": [{"userEnteredValue": str(threshold)}]}, "format": {"textFormat": {"foregroundColor": {"red": 1, "green": 1, "blue": 1}, "bold": True}}}}, "index": 0}})
    ...
    rules = [
        ...
        ("NUMBER_GREATER_THAN_OR_EQUAL", ["110"], "#0000ff", True)
    ]
    ...
    for c_idx, key in target_cols:
        ...
        fmt_rules = [
            ("NUMBER_LESS_THAN_OR_EQUAL", [str(steps[0])], "#ff0000", True),
            ...
            ("NUMBER_GREATER_THAN_OR_EQUAL", [str(steps[3])], "#0000ff", True)
        ]
    ...
    print("\n   -> 機種別分析 v4.2 完了 (UIプロ仕様 適用完了)")

async def main():
    print(f"\n--- Ver.4.2 起動 (UI Fix Edition) ---")
