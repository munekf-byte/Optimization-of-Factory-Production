# --- VERSION: m_commander_v3_8_AutoSync_Panorama_Fixed_20260201 ---
...
# ==========================================
# BLOCK: 2. 自動同期エンジン（店舗リスト）
# ==========================================
async def sync_store_list(doc, all_data):
    print("   > 店舗リストをCSVから自動同期中...")
    try:
        ...
        req = {
            "setDataValidation": {
                "range": {"sheetId": s_id, "startRowIndex": 4, "endRowIndex": 5, "startColumnIndex": 1, "endColumnIndex": 2},
                "rule": {
        ...
        print(f"   -> 同期完了: {len(unique_stores)}店舗を反映しました。")
    except Exception as e:
        print(f"   ! 店舗同期エラー: {e}")

# ==========================================
# BLOCK: 3. クロス分析エンジン（パノラマ・可変スケール）
# ==========================================
async def execute_cross_analysis(doc, conf, all_data):
    ...
    def build_panorama_chart(w_idx, window_val, anchor_row):
        ...
        return { "addChart": { "chart": { "spec": { "title": title, "basicChart": { "chartType": "LINE", "legendPosition": "BOTTOM_LEGEND",
                ...
                "position": { "overlayPosition": { "anchorCell": { "sheetId": s_id, "rowIndex": anchor_row, "columnIndex": 0 }, 
                "widthPixels": 5000, "heightPixels": 450 } } } } }
...
# ==========================================
async def main():
    print(f"\n--- Ver.3.8 起動 (AutoSync & Panorama) [{datetime.now().strftime('%H:%M:%S')}] ---")
    ...
    while True:
        try:
            ...
            if "実行" in str([all_cmd, single_cmd, cross_cmd]):
                ...
                # 条件分岐に基づき分析を実行
                if "実行" in all_cmd or "実行" in cross_cmd:
                    await execute_cross_analysis(doc, conf, all_data)
                
                if "実行" in all_cmd or "実行" in single_cmd:
