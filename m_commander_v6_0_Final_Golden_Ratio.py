# --- VERSION: m_commander_v6_0_Final_Golden_Ratio ---
# 1. 座標修正: ランキング起点を A, D, G, J へ
# 2. 期間拘束: B4/C4 取得ロジックの安定化
# 3. 書式死守: D:H列の背景色を一切上書きしない外科手術
# 4. グラフ復旧: 参照レンジの適正化
...
async def execute_single_analysis(doc, conf):
    ...
    try:
        s_str = tmp_ws.acell('B4').value
        e_str = tmp_ws.acell('C4').value
        start_limit = datetime.strptime(s_str, "%Y/%m/%d")
        end_limit = datetime.strptime(e_str, "%Y/%m/%d")
    except:
        print("   ! B4/C4の日付形式を確認してください。全期間で処理します。")
        start_limit, end_limit = datetime(2000,1,1), datetime(2099,12,31)

    # データ抽出 (v5.9 継承 3/5日ルール)
    ...
    s_id = ws.id
    # ゴースト対策（値のみ消去し、テンプレートの書式を維持）
    ws.batch_clear(['A81:ZZ2000'])
    ...
    for i, p in enumerate(periods):
        ...
        row = [[f"{p[0]}〜{p[-1]}", "", "", f"{int(p_ds/p_uc) if p_uc>0 else 0}枚", f"{(p_gs*3+p_ds)/(p_gs*3)*100 if p_gs>0 else 100:.1f}%", f"{int(p_gs/p_uc) if p_uc>0 else 0}G"]]
    ...
    for i, d_str in enumerate(target_dates):
        ...
        row = [d_str, ["月","火","水","木","金","土","日"][datetime.strptime(d_str, "%Y/%m/%d").weekday()], "", t_d, int(t_d/u_cnt), int(t_g/u_cnt), f"{(t_g*3+t_d)/(t_g*3)*100 if t_g>0 else 100:.1f}%", ""]
    ...
    # グラフ (一続きのデータ範囲を指定)
    reqs.append({"addChart": {"chart": {"spec": {"title": "トレンド", "basicChart": {"chartType": "LINE", "legendPosition": "BOTTOM_LEGEND", "axis": [{"position": "BOTTOM_AXIS"}, {"position": "LEFT_AXIS", "viewWindowOptions": {"viewWindowMin": 95, "viewWindowMax": 110, "viewWindowMode": "EXPLICIT"}}],
...
async def sync_store_list(doc):
    ...
    idx_ws.clear(); idx_ws.update(values=[["店舗リスト(AutoSync)"]] + [[s] for s in sorted(list(unique_stores))], range_name='A1')

async def main():
    print(f"\n--- Ver.6.0 起動 (Golden Ratio) ---")
