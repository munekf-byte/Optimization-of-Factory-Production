# --- VERSION: m_commander_v5_0_Pure_Logic ---
import gspread
...
async def execute_single_analysis(doc, conf):
    ...
    report_top = [
        ["【分析レポート】", conf['store'], conf['target_model']],
        ["期間", sorted_dates[0], "〜", sorted_dates[-1]],
        ["全体累計差枚", f"{total_diff}枚", "平均機械割", f"{overall_payout:.2f}%"]
    ]
    ...
    # 3-3. 台別ポテンシャル評価 (Phase 2)
    unit_summary_header = ["台番号評価", "10,000枚突破率", "5,000枚突破率", "台別平均差枚"]
    unit_summary_rows = []
    ...
    ws.update(values=[unit_summary_header] + unit_summary_rows, range_name='A8')
    ws.update(values=[data_header] + data_rows, range_name='A20')
    
    print("\n   -> 機種別分析 v5.0 完了 (Logic Only)")

async def main():
    print(f"\n--- Ver.5.0 起動 (Pure Logic & Dashboard) ---")
