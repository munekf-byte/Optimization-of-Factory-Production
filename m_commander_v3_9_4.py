# --- VERSION: m_commander_v3_9_4_Clean_Visual_Edition_20260205 ---
...
CROSS_SHEET     = "クロス分析"
SINGLE_SHEET    = "機種別分析"
# LOCAL_DATABASE  = "minrepo_database.csv" # ← リーダーが修正したパスをそのまま使ってください
...
async def execute_single_analysis(doc, conf, all_data):
    ...
    # --- 軍師レポート (1-15行目) ---
    avg_by_dow = {dow_names[i]: sum(weekday_stats[i])/len(weekday_stats[i]) if weekday_stats[i] else -9999 for i in range(7)}
    report = [
        [f"【軍師レポート】 {conf['target_model']} 戦略指南"],
        [f"最強曜日: {max(avg_by_dow, key=avg_by_dow.get)}曜日 / 最弱曜日: {min(avg_by_dow, key=avg_by_dow.get)}曜日"],
        [f"※ サマリーエリア（21-23行目）でポテンシャルの高い台を確認してください。"]
    ]

    # --- サマリー行 (21-23行目) ※C:G列は空白で汚染を防止 ---
    ...
    for u in sorted_units:
        ...
        days = len(diffs)
        if days > 0:
            summary_10k.append(f"{round(len([v for v in diffs if v>=10000])/days*100, 1)}%")
            summary_5k.append(f"{round(len([v for v in diffs if v>=5000])/days*100, 1)}%")
            summary_avg.append(int(sum(diffs)/days))
        else:
    ...
    def add_cf(formula, bg, text, bold=False):
        return {"addConditionalFormatRule": {"rule": {"ranges": [t_range], "booleanRule": {"condition": {"type": "CUSTOM_FORMULA", "values": [{"userEnteredValue": formula}]}, 
                "format": {"backgroundColor": hex_to_rgb(bg), "textFormat": {"foregroundColor": hex_to_rgb(text), "bold": bold}}}}, "index": 1}}
    
    reqs += [
        add_cf("=AND(ISNUMBER(H22), H22<=-3001)","#f4cccc","#ff0000",True),
        add_cf("=AND(ISNUMBER(H22), H22>=5000, H22<=7999)","#cfe2f3","#0000ff",True),
        add_cf("=AND(ISNUMBER(H22), H22>=8000)","#0b5394","#ffffff",True)
    ]
...
async def main():
    print(f"\n--- Ver.3.9.4 起動 (Clean Visual Edition) ---")
