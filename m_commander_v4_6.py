...
async def execute_single_analysis(doc, conf):
    ...
    for u in valid_units:
        ...
        days = len(diffs)
        v10, v5 = round(len([v for v in diffs if v>=10000])/days*100, 1) if days > 0 else 0, round(len([v for v in diffs if v>=5000])/days*100, 1) if days > 0 else 0
    ...
    for d_str in sorted_dates:
        ...
        for u in valid_units: row.append(day_data[u]['diff'] if u in data_rows else day_data[u]['diff'] if u in day_data else "")
    ...
    # 【修正】全ルールのメタデータを取得して一つずつ削除する
    full_ws_meta = doc.fetch_sheet_metadata()
    target_sheet_meta = next(s for s in full_ws_meta['sheets'] if s['properties']['sheetId'] == s_id)
    existing_rules_count = len(target_sheet_meta.get('conditionalFormats', []))

    # 基本リセット命令
    reqs = [{"updateCells": {"range": {"sheetId": s_id, "startRowIndex": 20, "endRowIndex": l_row, "startColumnIndex": 0, "endColumnIndex": l_col}, "fields": "userEnteredFormat"}}]
    # 条件付き書式を全削除
    for _ in range(existing_rules_count):
        reqs.append({"deleteConditionalFormatRule": {"index": 0, "sheetId": s_id}})
    ...
    for c_idx, key in [(2, 'diff_total'), (3, 'diff_avg'), (4, 'games_avg'), (6, 'win_rate')]:
        v_list = col_vals[key]; v_min, v_max = min(v_list), max(v_list)
        c_range = {"sheetId": s_id, "startRowIndex": 24, "endRowIndex": l_row, "startColumnIndex": c_idx, "endColumnIndex": c_idx+1}
        if key in ['diff_total', 'diff_avg']:
            reqs.append({"addConditionalFormatRule": {"rule": {"ranges": [c_range], "booleanRule": {"condition": {"type": "NUMBER_EQ", "values": [{"userEnteredValue": str(v_max)}]}, "format": {"textFormat": {"foregroundColor": hex_to_rgb("#0000ff"), "bold": True}}}}, "index": 0}})
            reqs.append({"addConditionalFormatRule": {"rule": {"ranges": [c_range], "booleanRule": {"condition": {"type": "NUMBER_GREATER", "values": [{"userEnteredValue": "0"}]}, "format": {"textFormat": {"foregroundColor": hex_to_rgb("#0000ff")}}}}, "index": 1}})
            reqs.append({"addConditionalFormatRule": {"rule": {"ranges": [c_range], "booleanRule": {"condition": {"type": "NUMBER_EQ", "values": [{"userEnteredValue": str(v_min)}]}, "format": {"textFormat": {"foregroundColor": hex_to_rgb("#ff0000"), "bold": True}}}}, "index": 2}})
            reqs.append({"addConditionalFormatRule": {"rule": {"ranges": [c_range], "booleanRule": {"condition": {"type": "NUMBER_LESS", "values": [{"userEnteredValue": "0"}]}, "format": {"textFormat": {"foregroundColor": hex_to_rgb("#ff0000")}}}}, "index": 3}})
        else:
            v_avg = sum(v_list)/len(v_list)
            reqs.append({"addConditionalFormatRule": {"rule": {"ranges": [c_range], "booleanRule": {"condition": {"type": "NUMBER_EQ", "values": [{"userEnteredValue": str(v_max)}]}, "format": {"textFormat": {"foregroundColor": hex_to_rgb("#0000ff"), "bold": True}}}}, "index": 0}})
            reqs.append({"addConditionalFormatRule": {"rule": {"ranges": [c_range], "booleanRule": {"condition": {"type": "NUMBER_GREATER", "values": [{"userEnteredValue": str(v_avg)}]}, "format": {"textFormat": {"foregroundColor": hex_to_rgb("#0000ff")}}}}, "index": 1}})
            reqs.append({"addConditionalFormatRule": {"rule": {"ranges": [c_range], "booleanRule": {"condition": {"type": "NUMBER_EQ", "values": [{"userEnteredValue": str(v_min)}]}, "format": {"textFormat": {"foregroundColor": hex_to_rgb("#ff0000"), "bold": True}}}}, "index": 2}})
            reqs.append({"addConditionalFormatRule": {"rule": {"ranges": [c_range], "booleanRule": {"condition": {"type": "NUMBER_LESS", "values": [{"userEnteredValue": str(v_avg)}]}, "format": {"textFormat": {"foregroundColor": hex_to_rgb("#ff0000")}}}}, "index": 3}})

    # UI5: 中央揃え
    reqs.append({"repeatCell": {"range": {"sheetId": s_id, "startRowIndex": 23, "endRowIndex": l_row, "startColumnIndex": 7, "endColumnIndex": l_col}, "cell": {"userEnteredFormat": {"horizontalAlignment": "CENTER", "verticalAlignment": "MIDDLE"}}, "fields": "userEnteredFormat(horizontalAlignment,verticalAlignment)"}})
