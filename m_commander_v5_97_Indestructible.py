# --- VERSION: m_commander_v5_97_Indestructible ---
# 1. 0除算エラーを完全回避 (Division by zero fix)
# 2. PMのスクリーンショットに基づき、座標をG3, J3, M3等へ精密固定
# 3. テンプレートの項目名を一切上書きしない「注入方式」を徹底
...
async def execute_single_analysis(doc, conf):
    ...
    ws.update(values=[[f"{avg_d}枚", "", "", f"{avg_g}G", "", "", f"{avg_r:.1f}%"]], range_name='G3:M3')
    ...
    for i, p in enumerate(p_res):
        ...
        row = [[f"{p['dates'][0]}〜{p['dates'][-1]}", "", "", f"{p_avg_d}枚", f"{p_avg_r:.1f}%", f"{p_avg_g}G"]]
        ws.update(values=row, range_name=f'H{9+i}:M{9+i}')
    ...
    for i, d_str in enumerate(target_dates):
        ...
        sticky_w = (len([u for u in day_data.values() if u['games']>=5000 and u['diff']>0])/u_cnt*100) if u_cnt > 0 else 0
        row = [d_str, dow_names[datetime.strptime(d_str, "%Y/%m/%d").weekday()], "", t_d, int(t_d/u_cnt), int(t_g/u_cnt), f"{day_avg_r:.1f}%", f"{sticky_w:.1f}%"]
