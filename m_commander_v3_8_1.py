# --- VERSION: m_commander_v3_8_1_Sync_Refined_20260202 ---
...
# ==========================================
# BLOCK: 2. 自動同期エンジン（改良版）
# ==========================================
async def sync_store_list(doc, all_data):
    print("   > 店舗リストを同期中...")
    try:
        ...
        try:
            ...
            idx_ws = doc.add_worksheet(title=INDEX_SHEET, rows=1000, cols=5)
        
        idx_ws.clear()
        store_values = [[s] for s in unique_stores]
        idx_ws.update(values=[["店舗リスト(AutoSync)"]] + store_values, range_name='A1')
        
        # 2. プルダウン（B5）の設定を「機種目録シートの範囲参照」に更新
        conf_ws = doc.worksheet(CONFIG_SHEET)
        conf_sheet_id = conf_ws.id
        
        # 店舗名の入っている範囲を動的に指定（例: 機種目録!A2:A50）
        range_formula = f"='{INDEX_SHEET}'!$A$2:$A${len(unique_stores) + 1}"
        
        req = {
            "setDataValidation": {
                "range": {
                    "sheetId": conf_sheet_id,
                    "startRowIndex": 4, "endRowIndex": 5, # 5行目 (B5)
                    "startColumnIndex": 1, "endColumnIndex": 2 # B列
                },
                "rule": {
                    "condition": {
                        "type": "VALUE_IN_RANGE", # 範囲参照に切り替え
        ...
        print(f"   -> 同期完了: {len(unique_stores)}店舗。B5セルを確認してください。")
...
async def execute_cross_analysis(doc, conf, all_data):
    ...
    ws.update(values=data_rows, range_name='U50')

    s_id = ws.id; l_row = len(data_rows) + 50
    ...
    def build_panorama_chart(w_idx, window_val, anchor_row):
        ...
        title = f"【{window_val}日MA】灰:全体 赤:{conf['A_name']} 青:{conf['B_name']} 緑:{conf['C_name']} (振切:{of_cnt}回)"
        series = []
        ...
        return { "addChart": { "chart": { "spec": { "title": title, "basicChart": { "chartType": "LINE", "legendPosition": "BOTTOM_LEGEND",
                ...
                "position": { "overlayPosition": { "anchorCell": { "sheetId": s_id, "rowIndex": anchor_row, "columnIndex": 0 }, "widthPixels": 5000, "heightPixels": 450 } } } }

    reqs = [
        {"repeatCell": {"range": {"sheetId": s_id, "startRowIndex": 49, "endRowIndex": l_row, "startColumnIndex": 2, "endColumnIndex": 18}, "cell": {"userEnteredFormat": {"numberFormat": get_number_format(conf['mode']), "horizontalAlignment": "CENTER"}}, "fields": "userEnteredFormat"}},
...
async def main():
    print(f"\n--- Ver.3.8.1 起動 (Sync Refined) [{datetime.now().strftime('%H:%M:%S')}] ---")
    ...
    while True:
        try:
            ...
            if "実行" in str([all_cmd, single_cmd, cross_cmd]):
                ...
                with open(LOCAL_DATABASE, mode='r', encoding='utf-8-sig') as f: all_data = list(csv.reader(f))
                await sync_store_list(doc, all_data)
                vals = conf_ws.get_all_values()
                conf = {
