# --- VERSION: m_commander_v3_9_6_API_Fix_20260205 ---
import gspread
...
# ==========================================
# BLOCK: 2. 超高速同期エンジン（API定数修正版）
# ==========================================
async def sync_store_list(doc):
    print(f"   > [{datetime.now().strftime('%H:%M:%S')}] 倉庫を高速スキャン中...")
    try:
        unique_stores = set()
        ...
        print(f"   -> 同期完了: {len(stores)}店舗。プルダウンを確認してください。")
    except Exception as e:
        print(f"   ! 同期エラー: {e}")

# ==========================================
# BLOCK: 3. 各分析エンジン（3.9.5 継承）
# ==========================================
...
async def execute_single_analysis(doc, conf):
    ...
    data_rows = [[d_str, dow_names[datetime.strptime(d_str, "%Y/%m/%d").weekday()], sum(u['diff'] for u in day.values()), sum(u['diff'] for u in day.values())/len(day), sum(u['games'] for u in day.values())/len(day), ((sum(u['games'] for u in day.values())*3+sum(u['diff'] for u in day.values()))/(sum(u['games'] for u in day.values())*3)*100) if sum(u['games'] for u in day.values())>0 else 0, len([u for u in day.values() if u['games']>=5000 and u['diff']>0])/len(day)] + [day[u]['diff'] if u in day else "" for u in sorted_units] for d_str in sorted_dates]
