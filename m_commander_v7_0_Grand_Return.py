# --- VERSION: m_commander_v8_0_Safety_Guard_Active ---
import gspread
...
async def execute_single_analysis(doc, conf):
    ...
    if not valid_units: 
        print("   ! 有効な正規台（3/5ルール適用）が見つかりませんでした。")
        return
    ...
    # --- STEP 4: Dashboard A1:O33 精密マッピング ---
    # 【ガード】全体平均
    ...
    for i, d_str in enumerate(target_dates):
        ...
        sticky_val = (len([u for u in day_data.values() if u['games']>=5000 and u['diff']>0])/u_cnt*100) if u_cnt > 0 else 0.0
    ...
    print(f"\n   -> {conf['target_model']} 解析完了 (Version 5.9.1)")

async def main():
    print(f"\n--- Ver.5.9.1 起動 (Safety Guard Edition) ---")
