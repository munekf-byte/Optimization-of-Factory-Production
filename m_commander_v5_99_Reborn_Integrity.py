# --- VERSION: m_commander_v5_99_Reborn_Integrity ---
# 1. v5.9継承: 3/5日ルールによるノイズ除去
# 2. v5.96継承: G3, J3, M3 を含む精密座標マッピング
# 3. 修正: ゴーストデータ根絶 (A81:ZZ2000の完全消去)
...
async def execute_single_analysis(doc, conf):
    print(f"   > 分析実行: {conf['target_model']}")
    
    # テンプレートから期間取得 (安全策)
    tmp_ws = doc.worksheet(TEMPLATE_SHEET)
    try:
        p_dates = tmp_ws.batch_get(['B4', 'C4'])
        start_limit = datetime.strptime(p_dates[0][0][0], "%Y/%m/%d")
        end_limit = datetime.strptime(p_dates[1][0][0], "%Y/%m/%d")
    except:
        print("   ! B4/C4の日付読み取りに失敗しました。デフォルト全期間で実行します。")
    ...
    s_id = ws.id
    ws.batch_clear(['A81:ZZ2000']) # 右側のゴーストを完全に消去
    ...
    def get_rank(dates):
        ...
        res = sorted([(u, int(sum(v)/len(v))) for u, v in u_s.items() if v], key=lambda x: x[1], reverse=True)
        return res[:5], res[-5:][::-1]

    periods = []
    if len(target_dates) >= 3:
        n = len(target_dates)
        periods = [target_dates[:n//3], target_dates[n//3:2*n//3], target_dates[2*n//3:]]
